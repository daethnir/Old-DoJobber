...
AVAILABLE_MOVIES = ['Noises Off', 'MST3K', 'Babylon 5']
...
class PickTimeAndDate(Job):
    ...
    TIMES = ('2024-04-08 18:18 UTC', '2099-09-14 16:57 UTC')
...
class ValidateMovie(Job):
    ...
    def Check(self, *_, **kwargs):
        if kwargs.get('movie') not in AVAILABLE_MOVIES:
            raise RuntimeError(
                f'{kwargs.get("movie")} not one of the available movies.'
            )
...
class SendInvite(Job):
    ...
    def Run(self, *_, **kwargs):
        ...
        """
        content = (
            f'Come and watch {kwargs["movie"]} at'
            f' {self.global_storage["Start-DateTime"]} with me!'
        )
        recipient = f'{self.NAME} <{self.EMAIL}>'
        ...
        #     [self.EMAIL]
        #     msg.as_string())
        logging.info('Sent %s the following: \n\t%s\n', recipient, content)
        self.SENT = True
...
def invite_friends(people):
    ...
    for person in people:
        job = type(f'Invite {person["name"]}', (SendInvite,), {})
...
def handle_args():
    ...
    group.add_argument(
        '-d', dest='debug', action='store_true', help='Debug DoJobber output.'
    )
    ...
    group.add_argument(
        ...
        f' Values higher than {Popcorn.TRIES} will cause this to fail, so'
        ' you can test the retry logic.',
    )
    ...
    group.add_argument(
        ...
        ' By default this contrived example will succeed on the last try.'
        f' Values higher than {PopcornBowl.TRIES} will cause this to fail, so'
        ' you can test the retry logic.',
    )

    group.add_argument(
        '--pizza_success_try',
        dest='pizza_success_try',
        type=int,
        metavar='#',
        help='Achieve sucess on the Pizza Job at this number of tries.'
        ' By default this contrived example will succeed on the last try.'
        f' Values higher than {Pizza.TRIES} will cause this to fail, so'
        ' you can test the retry logic.',
    )
    ...
    args.x11 = bool(os.environ.get('DISPLAY') and args.x11)
...
def main():
    ...
    if args.png_output:
        with open(args.png_output, 'w', encoding='utf8') as out:
